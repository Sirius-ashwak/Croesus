...
const env = (k: string): Address | undefined => {
  ...
  return v && v.length > 0 ? (v as Address) : undefined;
};

/** Deployed addresses (filled by Deploy.s.sol output -> .env). */
export const addresses = {
  registry: env("NEXT_PUBLIC_REGISTRY_ADDRESS"),
  mezoBorrow: env("NEXT_PUBLIC_MEZO_BORROW_ADDRESS"),
  musd: env("NEXT_PUBLIC_MUSD_TOKEN_ADDRESS"),
  tbtc: env("NEXT_PUBLIC_TBTC_TOKEN_ADDRESS"),
  pyth: env("NEXT_PUBLIC_PYTH_ORACLE_ADDRESS"),
} as const;
