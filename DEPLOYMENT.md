# Deploying Croesus to Vercel

Croesus is a **Turborepo + pnpm monorepo**. Only the Next.js 14 app at
`apps/web` (`@croesus/web`) is deployed to Vercel — the Foundry/Solidity
package at `packages/contracts` is **never** built on Vercel (no toolchain
there, and the build is scoped away from it).
